...
- [ ] `web/src/shared/`, `entities/admin-user/`, `features/admin-user-crud/`, `pages/admin-users/`(또는 기존 pages 하위) 구조가 준비되어 있다.

---

### 2. shared — API 베이스·타입

- **파일**: `web/src/shared/api/client.ts` (또는 base fetch), `web/src/shared/lib/types.ts` 또는 `shared/api/admin-user/types.ts`
- **작업**:
  - api-spec과 일치하는 AdminUser 타입 정의 (id, username, created_at).
  - fetch 래퍼 또는 axios 인스턴스( baseURL, JSON 헤더 ). (선택) 공통 에러 처리.
- **체크**
  - [ ] AdminUser 타입이 정의되어 있다.
  - [ ] API 베이스 URL·헤더 설정이 한 곳에 있다.

---

### 3. shared — Admin User API 함수

- **파일**: `web/src/shared/api/admin-user.ts` (또는 entities/admin-user/api)
- **작업**: getList(), getById(id), create({ username, password }), update(id, { username?, password? }), delete(id). 반환은 api-spec 응답 형식에 맞춤.
- **체크**
  - [ ] GET/POST/PUT/DELETE에 대응하는 함수가 있고, api-spec 경로·body와 일치한다.
  - [ ] 에러 시 throw 또는 Result 타입으로 처리한다.

---

### 4. entities/admin-user — 모델·UI 조각(선택)

- **파일**: `web/src/entities/admin-user/model/types.ts` (AdminUser 재export 또는 확장), (선택) `ui/AdminUserRow.tsx`
- **작업**: AdminUser 타입을 entities에서 export. 목록 행 컴포넌트가 필요하면 id, username, created_at 표시 + 수정/삭제 버튼(클릭은 features에서 처리).
- **체크**
  - [ ] entities/admin-user에서 AdminUser 타입을 사용할 수 있다.
  - [ ] (선택) 행 UI가 있고, 상위에서 onEdit, onDelete 등을 받는다.

---

### 5. features/admin-user-crud — 목록 조회

- **파일**: `web/src/features/admin-user-crud/ui/AdminUserList.tsx` (또는 model/useAdminUserList.ts + ui)
- **작업**:
...
- **체크**
  - [ ] username, password 입력과 제출 시 POST가 호출된다.
  - [ ] 성공 후 목록 갱신 또는 네비게이션된다.
  - [ ] 에러 메시지를 사용자에게 보여준다.
...
- **체크**
  - [ ] 수정 폼에서 기존 username이 로드되어 표시된다.
  - [ ] 제출 시 PUT이 호출되고, 성공 후 화면이 갱신된다.
  - [ ] 404/409 에러를 처리한다.
...
- **파일**: `web/src/features/admin-user-crud/ui/DeleteAdminUserButton.tsx` (또는 모달 포함)
- **작업**: “삭제” 클릭 시 확인 다이얼로그. 확인 후 delete(id) 호출. 성공 시 목록에서 제거 또는 목록 새로고침.
- **체크**
  - [ ] 삭제 전 확인 UI가 있다.
  - [ ] 확인 시 DELETE가 호출되고, 성공 시 목록이 갱신된다.

---

### 9. pages/admin-users — CRUD 페이지 조합

- **파일**: `web/src/pages/AdminUsersPage.tsx`를 FSD에 맞게 수정하거나, `web/src/pages/admin-users/index.tsx`(또는 AdminUsersPage.tsx)에서 features 조합.
- **작업**: 목록 + “추가” 버튼 → 생성 폼(모달 또는 별도 라우트). 행 “수정” → 수정 폼. 행 “삭제” → 삭제 확인. 라우트는 `/admin/users`, (선택) `/admin/users/new`, `/admin/users/:id/edit`.
- **체크**
  - [ ] 한 페이지(또는 라우트 조합)에서 목록·생성·수정·삭제가 모두 동작한다.
  - [ ] 기존 AdminUsersPage가 FSD 구조(entities/features)를 사용하도록 정리되어 있다.
...
- **체크**
  - [ ] `/admin/users` 접근 시 Admin User CRUD 페이지가 열린다.
...
- [ ] shared에 Admin User API 클라이언트와 타입이 있고, api-spec과 일치한다.
- [ ] entities/admin-user에 타입(및 선택적 UI)이 있다.
- [ ] features/admin-user-crud에서 목록·생성·수정·삭제가 구현되어 있다.
- [ ] pages에서 위 feature들을 조합한 CRUD 페이지가 있다.
- [ ] app 라우터에 Admin User 관련 경로가 등록되어 있다.
- [ ] 로딩·에러 상태가 적절히 표시된다.
- [ ] 기존 목록 동작이 유지되며, 생성·수정·삭제가 정상 동작한다.
