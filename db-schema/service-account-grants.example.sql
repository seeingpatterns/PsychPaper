...
-- - ENUM·테이블·시퀀스·함수를 나열하지 않고 `public` 스키마 단위로 부여합니다.
...
-- 4) 이미 존재하는 public 객체 일괄 부여 (ENUM·테이블·시퀀스·함수)
--    새 ENUM/테이블/시퀀스/함수 추가 시마다 역할을 수정할 필요 없음.
--    `ALL FUNCTIONS`에는 uuid-ossp 등 extension이 public에 둔 함수도 포함됩니다.
--    앱 전용 DB·앱 객체만 public에 둔다는 전제에서 사용하세요.

GRANT USAGE ON ALL TYPES IN SCHEMA public TO psychpaper_app;
...
GRANT EXECUTE ON ALL FUNCTIONS IN SCHEMA public TO psychpaper_app;
...
ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA public
  GRANT EXECUTE ON FUNCTIONS TO psychpaper_app;

-- 저장 프로시저(PROCEDURE)를 도입하면 사용 중 PostgreSQL 버전 문서에 따라
-- ROUTINES/PROCEDURE에 대한 GRANT·DEFAULT PRIVILEGES를 추가로 검토하세요.
